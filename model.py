...
class LightGCN(BasicModel):
    ...
    def css_loss(self, users, pos, neg):
        ...
        pos_scores = t.mul(users_emb, pos_emb)
        pos_scores = t.sum(pos_scores, dim=1)
        negs_scores = t.mul(users_emb.unsqueeze(1), negs_emb)
        negs_scores = t.sum(negs_scores, dim=2)
        negs_scores = t.mean(negs_scores, dim=1)

        loss = t.mean(t.nn.functional.softplus(negs_scores - pos_scores))
...
class MGRF(BasicModel):
    ...
    def __init_weight(self):
        ...
        self.embedding_user = t.nn.Embedding(num_embeddings=self.n_user, embedding_dim=self.embed_size)
        self.embedding_item = t.nn.Embedding(num_embeddings=self.n_item, embedding_dim=self.embed_size)
        self.weight_user = t.nn.Embedding(num_embeddings=self.n_user, embedding_dim=self.n_item)
        self.weight_item = t.nn.Embedding(num_embeddings=self.n_item, embedding_dim=self.n_user)
        if self.args.pretrain:
            self.embedding_user.weight.data.copy_(t.from_numpy(self.config['user_emb']))
            self.embedding_item.weight.data.copy_(t.from_numpy(self.config['item_emb']))
            print('use pretarined data')
            ...
            nn.init.normal_(self.embedding_user.weight, std=0.1)
            nn.init.normal_(self.embedding_item.weight, std=0.1)
            nn.init.normal_(self.weight_user.weight, std=0.1)
            nn.init.normal_(self.weight_item.weight, std=0.1)
            cprint('use NORMAL distribution initilizer')
        self.f = nn.Sigmoid()
        self.Graph = self.dataset.getSparseGraph()
    ...
    def computer(self):
        """
        propagate methods for MGRF
        """
        users_emb = self.embedding_user.weight
        items_emb = self.embedding_item.weight
        users_weight = self.weight_user.weight
        items_weight = self.weight_item.weight
        all_emb = t.cat([users_emb, items_emb])
        ...
        for layer in range(self.layer):
            ...
            #cross
            users, items = t.split(side_emb, [self.n_user, self.n_item])

            users = t.tanh(torch.matmul(users_weight, items))
            items = t.tanh(torch.matmul(items_weight, users))

            side_emb = t.cat([users, items])
            side_emb = side_emb + all_emb if self.args.residual else side_emb

            # multi-graph
            similarity_emb = t.sparse.mm(similarity, all_emb)

            all_emb = (side_emb + similarity_emb) / 2
