...
class LightGCN(Model):
    ...
    def call(self, inputs):
        ...
        pos_embed = tf.nn.embedding_lookup(self.item_embedding, pos)
        neg_embed = tf.nn.embedding_lookup(self.item_embedding, neg)
        print(user_embed)
        print(self.user_embedding.trainable)
        print(self.item_embedding.trainable)
        self.ego_embed = tf.concat([self.user_embedding, self.item_embedding], axis=0)
        print(self.ego_embed.tr)
        # all_embed = [ego_embed]
