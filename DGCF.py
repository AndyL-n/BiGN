...
class DGCF(nn.Module):
    def __init__(self, data_config, pretrain_data):
        ...
        self.n_users = data_config['n_users']   # user数量
        self.n_items = data_config['n_items']   # item数量
    ...
    def __init_weight(self):
        self.embedding_user = t.nn.Embedding(num_embeddings=self.n_users, embedding_dim=self.emb_dim)
        self.embedding_item = t.nn.Embedding(num_embeddings=self.n_items, embedding_dim=self.emb_dim)
    ...
    def _convert_A_values_to_A_factors_with_P(self, f_num, A_factor_values, pick=True):
        ...
        if pick:
            ...
            index = index.type(torch.float32) * (self.pick_level - 1.0) + 1.0
            # adjust the weight of the minimum factor to 1/self.pick_level
        ...
        for i in range(0, f_num):
            ...
            A_i_tensor = torch.sparse_coo_tensor(indices=[self.all_h_list, self.all_t_list], values=A_i_scores,size=self.A_in_shape)
            ...
            D_i_col_tensor = torch.sparse_coo_tensor(
                indices=[list(range(self.n_users + self.n_items)), list(range(self.n_users + self.n_items))],
                values=D_i_col_scores, size=self.A_in_shape)
            D_i_row_tensor = torch.sparse_coo_tensor(
                indices=[list(range(self.n_users + self.n_items)), list(range(self.n_users + self.n_items))],
                values=D_i_row_scores, size=self.A_in_shape)
    ...
    def _create_star_routing_embed_with_P(self, pick_=False):
        ...
        #
        factor_num = [self.n_factors for _ in range(self.n_layers)]

        # T 迭代次数
        iter_num = [self.n_iterations for _ in range(self.n_layers)]
        ...
        for k in range(0, self.n_layers):
            ...
            # tensorflow 和 torch split 区别
            ego_layer_embeddings = torch.split(ego_embeddings, self.emb_dim//n_factors_l, 1)
            ego_layer_embeddings_t = torch.split(ego_embeddings, self.emb_dim//n_factors_l, 1)
            ...
            for t in range(0, n_iterations_l):
                ...
                for i in range(0, n_factors_l):
                    ...
                    print(D_col_factors[i].shape)
                    print(ego_layer_embeddings[i].shape)
                    # 第K个因素 对每个
                    ...
                    print(factor_embeddings.shape)
                    print(factor_embeddings_t.shape)
                    ...
                    print(head_factor_embedings.shape)
                    exit()
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    print(len(all_h_list))  # 行
    print(len(all_t_list))  # 列
    print(len(all_v_list))  # 值

    A_values_init = create_initial_A_values(args.n_factors, all_v_list)
    print(A_values_init.shape)
    ...
    ***********************************************************
    ...
    model = DGCF(data_config=config, pretrain_data=pretrain_data).to(device)
